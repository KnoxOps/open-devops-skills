...
| 6 | Access logging enabled | Logs show zero access events over 90 days -> confirms IDLE | CloudTrail data events / GCS access logs |
...
| Scenario | Treatment |
|----------|-----------|
| Logging/audit buckets | Data arrives externally (ALB/CloudTrail push). Zero GETs is normal — classify by PUT activity only. IDLE only if 0 PUTs AND 0 objects in 90 days |
| Static website hosting | Check CloudFront/CDN access logs, not bucket-level GETs. If S3 website endpoint is enabled but no CDN log activity exists, consider IDLE |
...
| Buckets used by serverless triggers | S3 event notifications to Lambda/SQS/SNS mean activity happens off-bucket. Check trigger invocation logs, not just bucket metrics |
