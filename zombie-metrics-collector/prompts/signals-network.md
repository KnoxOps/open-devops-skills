...
|---|----------|--------|------------------|-------------|
| 1 | EIP | AssociationStatus | Not associated with any ENI or instance | AWS EC2 DescribeAddresses / cloud provider API |
| 2 | EIP | InheritCompute | Associated instance is zombie -> inherit INACTIVE | Cross-reference compute signal (signals-compute.md) |
...
| Parent | Child | Rule |
|--------|-------|------|
| Compute instance | Associated EIP | Compute idle -> EIP idle (unless EIP has other associations) |
| Target group | ALB/NLB | All targets unhealthy -> LB idle |
...
- **EIP on stopped instance**: Instance may be temporarily stopped for maintenance. Check stop duration — idle only if instance stopped > 30 days.
