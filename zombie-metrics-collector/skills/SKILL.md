...
Discover all cloud resources in scope using available cloud CLI tools.

**Input files**:
- {run_dir}/scan_plan.json: collection_templates[] and scope_filters (region, tag)
- {run_dir}/intent_detection.json: entity_types[] to scan

**Execution**:
1. Read intent_detection.json → entity_types[]
2. Read scan_plan.json → collection_templates[], scope_filters
3. For each entity_type, discover all resources in scope via cloud CLI tools
4. For each resource, extract: resource_id, resource_name, entity_type, resource_type (compute/storage/network/database/cache/k8s_workload/k8s_service/k8s_orphan/domain/object_storage), environment, tags, provisioned_at, private_ip, status, spec, cost
5. For compute resources: detect `virt_type` (lxc/kvm/bare-metal/unknown)

**Output**:
- Write {run_dir}/layer_a_raw_resources.json (all resources, unfiltered)
- Update {run_dir}/scan_episodes.json:
  scan_status: "phase1_layer_a1_completed"
  current_layer: "A.1"
  layer_a_stats.total_in_scope: total count

**Error handling**: If a tool for one entity_type fails completely, skip it and continue. If all fail, abort.
...
1. New resources: provisioned_at within last 7 days -> exclude, reason="new_resource"
...
|--------------|---------------|
| compute | `prompts/signals-compute.md` |
| database, cache | `prompts/signals-database.md` |
| storage | `prompts/signals-storage.md` |
| network | `prompts/signals-network.md` |
| k8s_workload | `prompts/signals-k8s-workload.md` |
| k8s_service, k8s_orphan | `prompts/signals-k8s-service.md` |
| object_storage | `prompts/signals-object-storage.md` |
| domain | no signals available; mark unmeasurable, skip to manual review |

**CRITICAL — Threshold source**: All signal thresholds, units, and collection methods come ONLY from the `prompts/signals-{type}.md` reference files. DO NOT use thresholds from scan_plan.json, intent_detection.json, or any other file. DO NOT add extra signals (memory, disk_io, connections) beyond what the reference file defines. Collect ONLY the signals listed in the table above.
