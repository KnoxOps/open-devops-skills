---
name: zombie-resource-cleaner
description: Phase 6 Delete - Zombie resource deletion execution. Orchestrates a 6-step deletion process: pre-flight re-evaluation -> verify backup-creator backups -> deletion execution -> deletion verification -> metadata update -> audit + cost report. Supports batch deletion (10 per batch, 5-minute intervals), cascading cleanup across resource types, special EIP DNS check, complete audit trail and cost savings analysis. Backups are performed upfront by zombie-backup-creator; resource-cleaner only verifies that backups exist.
---
...
| enable_eip_dns_check | boolean | No | Enable DNS record check before EIP deletion |
...
For each batch (sequentially):
  For each resource in batch (up to {deletion_parallelism} in parallel):
    ...
    1. Detach traffic
       - Remove from SLB backends
       - Clear DNS records
       - Verify K8s endpoints empty
       - Unbind EIP (release separately)
    ...
    4. Delete main resource
       ...
       - Special for EIP: check DNS records before release (if {enable_eip_dns_check})
